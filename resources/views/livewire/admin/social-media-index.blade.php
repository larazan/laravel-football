<div class="vs jj ttm vl ou uf na">
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">Social Medias ✨</h1>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Social Medias <span class="gq gp"></span></h2>
        </header>
                ...
                <table class="ux ou">
                    ...
                    <tbody class="text-sm le lr">
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $socmed->link }}</div>
                            </td>
                            
                            <td class="vi wy w_ vo lm">
                                @if ($socmed->status === 'inactive')
                                <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $socmed->status }}</div>
                                @endif

                                @if ($socmed->status === 'active')
                                <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $socmed->status }}</div>
                                @endif
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div>{{ $socmed->created_at->format('d-m-Y') }}</div>
                            </td>

                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $socmed->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteId({{ $socmed->id }})">
    ...
    <x-dialog-modal wire:model="showSocialMediaModal" class="">

        @if ($socmedId)
        <x-slot name="title" class="border-b">Update Social Media</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create Social Media</span>
        </x-slot>
        @endif
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-button wire:click="closeSocialMediaModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-button>
                    @if ($socmedId)
                    <x-button wire:click="updateSocialMedia" class=" ho xi ye">Update</x-button>
                    @else
                    <x-button wire:click="createSocialMedia" class=" ho xi ye2">Create</x-button>
                    @endif
                </div>
            </div>
