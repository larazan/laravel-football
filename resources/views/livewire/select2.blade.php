...
>
  ...
  <div class="relative">
    <button
      wire:click="toggle"
      class="w-full flex items-center justify-between h-12 bg-white border border-2 rounded px-2"
    ...
    @if ($open)
      <ul class="bg-white absolute mt-1 z-10 border rounded w-full">
        @foreach($items as $item)
          <li wire:click="select({{ $loop->index }})"
            ...
            :class="{'bg-indigo-500 text-white': index === highlighted}"
            @mouseover="highlighted = index"
          >
            {{ $item }}

            @if ($selected === $loop->index)
              <div :class="true ? 'text-white' : 'text-indigo-500'">
