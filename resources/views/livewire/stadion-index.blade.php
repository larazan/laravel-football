...
<x-loading-indicator />
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">Stadion ✨</h1>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Stadions <span class="gq gp"></span></h2>
        </header>
                    ...
                    <tbody class="text-sm le lr">
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $stadion->name }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp ">
                                    @if ($stadion->small)
                                    <img src="{{ asset('storage/'.$stadions->small) }}" class="object-scale-down h-48 w-96" alt="{{ $stadion->name }}">
                                    @endif    
                                </div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                <div class="gp text-slate-800">{{ $stadion->capacity }}</div>
                            </td>
                            <td class="vi wy w_ vo lm">
                                @if ($stadion->status === 'inactive')
                                    <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $stadion->status }}</div>
                                @endif 

                                @if ($stadion->status === 'active')
                                    <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $stadion->status }}</div>
                                @endif 
                            </td>

                            <td class="vi wy w_ vo lm">
                                <div>{{ $stadion->created_at->format('d-m-Y') }}</div>
                            </td>

                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $stadion->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteId({{ $stadion->id }})">
    ...
    <x-jet-dialog-modal wire:model="showStadionModal" class="">

        @if ($stadionId)
        <x-slot name="title" class="border-b">Update Stadion</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create Stadion</span>
        </x-slot>
        @endif
                                    ...
                                    <div class="flex flex-col space-y-3">
                                        ...
                                        <div class="flex flex-row justify-between">
                                            <div class="col-start-1 sm:col-span-3">
                                                ...
                                                <input wire:model="city" type="text" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                            </div>
                                            <div class="col-start-1 sm:col-span-3">
                                        ...
                                        <div class="col-span-6 sm:col-span-3">
                                            ...
                                            <select wire:model="stadionStatus" class="h-full rounded-r border-t border-r border-b block appearance-none w-full bg-white border-gray-300 text-gray-700 py-2 px-4 pr-8 leading-tight focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
                                                <option value="" >Select Option</option>
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-m-button wire:click="closeStadionModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-m-button>
                    @if ($stadionId)
                    <x-m-button wire:click="updateStadion" class=" ho xi ye">Update</x-m-button>
                    @else
                    <x-m-button wire:click="createStadion" class=" ho xi ye2">Create</x-m-button>
                    @endif
                </div>
            </div>
