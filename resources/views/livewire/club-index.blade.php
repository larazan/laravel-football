...
<x-loading-indicator />
    ...
    <div class="je jd jc ii">
        ...
        <!-- Left: Title -->
        <div class="ri _y">
            <h1 class="gu teu text-slate-800 font-bold">Club ✨</h1>
        </div>
        ...
        <div class="sn am jo az jp ft">
            ...
            <button class="btn ho xi ye" wire:click="showCreateModal">
                ...
                <span class="hidden trm nq">Create Club</span>
            </button>
        </div>
    ...
    <div class="bg-white bd rounded-sm border border-slate-200 rc">
        <header class="vc vu">
            <h2 class="gh text-slate-800">Clubs <span class="gq gp"></span></h2>
        </header>
                        ...
                        <tr>
                            ...
                            <td class="vi wy w_ vo lm">
                                @if ($club->status === 'inactive')
                                    <div class="inline-flex gp hf yl rounded-full gn vp vd">{{ $club->status }}</div>
                                @endif 

                                @if ($club->status === 'active')
                                    <div class="inline-flex gp hc ys rounded-full gn vp vd">{{ $club->status }}</div>
                                @endif 
                            </td>

                            <td class="vi wy w_ vo lm">
                                <div>{{ $club->created_at->format('d-m-Y') }}</div>
                            </td>

                            <td class="vi wy w_ vo lm of">
                                <div class="fm">
                                    <button class="gq xv rounded-full" wire:click="showEditModal({{ $club->id }})">
                                    ...
                                    <button class="yl xy rounded-full" wire:click="deleteId({{ $club->id }})">
    ...
    <x-jet-dialog-modal wire:model="showClubModal" class="">

        @if ($clubId)
        <x-slot name="title" class="border-b">Update Club</x-slot>
        @else
        <x-slot name="title" class="border-b bg-slate-200">
            <span class="font-semibold">Create Club</span>
        </x-slot>
        @endif
                            ...
                            <div class="">
                                <div class="">
                                    <div class="flex flex-col space-y-3">
                                        ...
                                        <div class="col-span-6 sm:col-span-3">
                                            <label for="stadion" class="block text-sm font-medium text-gray-700">Stadion</label>
                                                <select wire:model="stadion" class="h-full2 rounded-r border-t border-r border-b block appearance-none w-full bg-white border-gray-300 text-gray-700 py-2 px-4 pr-8 leading-tight focus:placeholder-gray-600 focus:text-gray-700 focus:outline-none">
        ...
        <x-slot name="footer">
            <div class="border-slate-200">
                <div class="flex flex-wrap justify-end fc">
                    <x-m-button wire:click="closeClubModal" class="border-slate-200 hover:text-white hover--border-slate-300 g_">Cancel</x-m-button>
                    @if ($clubId)
                    <x-m-button wire:click="updateClub" class=" ho xi ye">Update</x-m-button>
                    @else
                    <x-m-button wire:click="createClub" class=" ho xi ye2">Create</x-m-button>
                    @endif
                </div>
            </div>
