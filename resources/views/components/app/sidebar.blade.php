        ...
        <div class="ff">
            ...
            <div>
                ...
                <ul class="nk">
                    ...
                    <!-- Competition -->
                    <li class="vn vr rounded-sm n_ ww @if(in_array(Request::segment(2), ['competitions'])){{ 'bg-slate-900' }}@else{{ '' }}@endif">
                        <a class="block gj xc ld wt wi" href="{{ url('admin/competitions') }}">
                            <div class="flex items-center">
                                <svg class="ub so oi" viewBox="0 0 24 24">
                                    <path class="du @if(in_array(Request::segment(2), ['competitions'])){{ 'text-indigo-600' }}@else{{ 'gz' }}@endif" d="M4.418 19.612A9.092 9.092 0 0 1 2.59 17.03L.475 19.14c-.848.85-.536 2.395.743 3.673a4.413 4.413 0 0 0 1.677 1.082c.253.086.519.131.787.135.45.011.886-.16 1.208-.474L7 21.44a8.962 8.962 0 0 1-2.582-1.828Z"></path>
                                    <path class="du @if(in_array(Request::segment(2), ['competitions'])){{ 'text-indigo-500' }}@else{{ 'g_' }}@endif" d="M10.034 13.997a11.011 11.011 0 0 1-2.551-3.862L4.595 13.02a2.513 2.513 0 0 0-.4 2.645 6.668 6.668 0 0 0 1.64 2.532 5.525 5.525 0 0 0 3.643 1.824 2.1 2.1 0 0 0 1.534-.587l2.883-2.882a11.156 11.156 0 0 1-3.861-2.556Z"></path>
                                    <path class="du @if(in_array(Request::segment(2), ['competitions'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" d="M21.554 2.471A8.958 8.958 0 0 0 18.167.276a3.105 3.105 0 0 0-3.295.467L9.715 5.888c-1.41 1.408-.665 4.275 1.733 6.668a8.958 8.958 0 0 0 3.387 2.196c.459.157.94.24 1.425.246a2.559 2.559 0 0 0 1.87-.715l5.156-5.146c1.415-1.406.666-4.273-1.732-6.666Zm.318 5.257c-.148.147-.594.2-1.256-.018A7.037 7.037 0 0 1 18.016 6c-1.73-1.728-2.104-3.475-1.73-3.845a.671.671 0 0 1 .465-.129c.27.008.536.057.79.146a7.07 7.07 0 0 1 2.6 1.711c1.73 1.73 2.105 3.472 1.73 3.846Z"></path>
                                </svg>
                    ...
                    <!-- Clubs -->
                    <li class="vn vr rounded-sm n_ ww @if(in_array(Request::segment(2), ['clubs'])){{ 'bg-slate-900' }}@else{{ '' }}@endif">
                        <a class="block gj xc ld wt wi" href="{{ url('admin/clubs') }}">
                            <div class="flex items-center">
                                    <svg class="ub so oi" viewBox="0 0 24 24">
                                        <path class="du @if(in_array(Request::segment(2), ['clubs'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" d="M13 6.068a6.035 6.035 0 0 1 4.932 4.933H24c-.486-5.846-5.154-10.515-11-11v6.067Z"></path>
                                        <path class="du @if(in_array(Request::segment(2), ['clubs'])){{ 'text-indigo-500' }}@else{{ 'gz' }}@endif" d="M18.007 13c-.474 2.833-2.919 5-5.864 5a5.888 5.888 0 0 1-3.694-1.304L4 20.731C6.131 22.752 8.992 24 12.143 24c6.232 0 11.35-4.851 11.857-11h-5.993Z"></path>
                                        <path class="du @if(in_array(Request::segment(2), ['clubs'])){{ 'text-indigo-600' }}@else{{ 'g_' }}@endif" d="M6.939 15.007A5.861 5.861 0 0 1 6 11.829c0-2.937 2.167-5.376 5-5.85V0C4.85.507 0 5.614 0 11.83c0 2.695.922 5.174 2.456 7.17l4.483-3.993Z"></path>
                                    </svg>
                     ...
                     <!-- Awards -->
                     <li class="vn vr rounded-sm n_ ww @if(in_array(Request::segment(2), ['awards'])){{ 'bg-slate-900' }}@else{{ '' }}@endif">
                        <a class="block gj xc ld wt wi" href="{{ url('admin/awards') }}">
                            <div class="flex items-center">
                                <svg class="ub so oi" viewBox="0 0 24 24">
                                    <path class="du @if(in_array(Request::segment(2), ['awards'])){{ 'text-indigo-500' }}@else{{ 'g_' }}@endif" d="M20 7a.75.75 0 01-.75-.75 1.5 1.5 0 00-1.5-1.5.75.75 0 110-1.5 1.5 1.5 0 001.5-1.5.75.75 0 111.5 0 1.5 1.5 0 001.5 1.5.75.75 0 110 1.5 1.5 1.5 0 00-1.5 1.5A.75.75 0 0120 7zM4 23a.75.75 0 01-.75-.75 1.5 1.5 0 00-1.5-1.5.75.75 0 110-1.5 1.5 1.5 0 001.5-1.5.75.75 0 111.5 0 1.5 1.5 0 001.5 1.5.75.75 0 110 1.5 1.5 1.5 0 00-1.5 1.5A.75.75 0 014 23z"></path>
                                    <path class="du @if(in_array(Request::segment(2), ['awards'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" d="M17 23a1 1 0 01-1-1 4 4 0 00-4-4 1 1 0 010-2 4 4 0 004-4 1 1 0 012 0 4 4 0 004 4 1 1 0 010 2 4 4 0 00-4 4 1 1 0 01-1 1zM7 13a1 1 0 01-1-1 4 4 0 00-4-4 1 1 0 110-2 4 4 0 004-4 1 1 0 112 0 4 4 0 004 4 1 1 0 010 2 4 4 0 00-4 4 1 1 0 01-1 1z"></path>
                                </svg>
                                <span class="text-sm gp ml-3 ttw tnn 2xl:opacity--100 wr">Awards</span>
                            </div>
                        </a>
                    </li>
                    <!-- Persons -->
                    <li class="vn vr rounded-sm n_ ww @if(in_array(Request::segment(2), ['players', 'staffs'])){{ 'bg-slate-900' }}@else{{ '' }}@endif" x-data="{ open: false }">
                        <a class="block gj xc ld wt wi" href="#0" @click.prevent="sidebarExpanded ? open = !open : sidebarExpanded = true">
                            <div class="flex items-center fe">
                                <div class="flex items-center">
                                    <svg class="ub so oi" viewBox="0 0 24 24">
                                        <path class="du @if(in_array(Request::segment(2), ['players', 'staffs'])){{ 'text-indigo-500' }}@else{{ 'g_' }}@endif" d="M18.974 8H22a2 2 0 012 2v6h-2v5a1 1 0 01-1 1h-2a1 1 0 01-1-1v-5h-2v-6a2 2 0 012-2h.974zM20 7a2 2 0 11-.001-3.999A2 2 0 0120 7zM2.974 8H6a2 2 0 012 2v6H6v5a1 1 0 01-1 1H3a1 1 0 01-1-1v-5H0v-6a2 2 0 012-2h.974zM4 7a2 2 0 11-.001-3.999A2 2 0 014 7z"></path>
                                        <path class="du @if(in_array(Request::segment(2), ['players', 'staffs'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" d="M12 6a3 3 0 110-6 3 3 0 010 6zm2 18h-4a1 1 0 01-1-1v-6H6v-6a3 3 0 013-3h6a3 3 0 013 3v6h-3v6a1 1 0 01-1 1z"></path>
                                    </svg>
                                    <span class="text-sm gp ml-3 ttw tnn 2xl:opacity--100 wr">Person</span>
                                </div>
                                <!-- Icon -->
                                <div class="flex shrink-0 ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                                    <svg class="w-3 h-3 shrink-0 ml-1 fill-current text-slate-400 @if(in_array(Request::segment(2), ['persons', 'nationalities'])){{ 'rotate-180' }}@endif" :class="open ? 'rotate-180' : 'rotate-0'" viewBox="0 0 12 12">
                        ...
                        <div class="tex ttj 2xl:block">
                            <ul class="me re @if(!in_array(Request::segment(2), ['players', 'staffs'])){{ 'hidden' }}@else{ !block }@endif" :class="open ? '!block' : 'hidden'">
                                <li class="rt ww">
            ...
            <div>
                ...
                <ul class="nk">
                    ...
                    <!-- Utility -->
                    <li class="vn vr rounded-sm n_ ww @if(in_array(Request::segment(2), ['faqs', 'slides', 'reports', 'subscribers'])){{ 'bg-slate-900' }}@else{{ '' }}@endif" x-data="{ open: false }">
                        <a class="block gj xc ld wt wi" href="#0" @click.prevent="sidebarExpanded ? open = !open : sidebarExpanded = true">
                            <div class="flex items-center fe">
                                <div class="flex items-center">
                                    <svg class="ub so oi" viewBox="0 0 24 24">
                                        <circle class="du @if(in_array(Request::segment(2), ['faqs', 'slides', 'reports', 'subscribers'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" cx="18.5" cy="5.5" r="4.5"></circle>
                                        <circle class="du @if(in_array(Request::segment(2), ['faqs', 'slides', 'reports', 'subscribers'])){{ 'text-indigo-500' }}@else{{ 'g_' }}@endif" cx="5.5" cy="5.5" r="4.5"></circle>
                                        <circle class="du @if(in_array(Request::segment(2), ['faqs', 'slides', 'reports', 'subscribers'])){{ 'text-indigo-500' }}@else{{ 'g_' }}@endif" cx="18.5" cy="18.5" r="4.5"></circle>
                                        <circle class="du @if(in_array(Request::segment(2), ['faqs', 'slides', 'reports', 'subscribers'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" cx="5.5" cy="18.5" r="4.5"></circle>
                                    </svg>
                                    <span class="text-sm gp ml-3 ttw tnn 2xl:opacity--100 wr">Utility</span>
                                </div>
                                <!-- Icon -->
                                <div class="flex shrink-0 ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                                    <svg class="w-3 h-3 shrink-0 ml-1 fill-current text-slate-400 @if(in_array(Request::segment(2), ['faqs', 'slides', 'reports', 'subscribers'])){{ 'rotate-180' }}@endif" :class="open ? 'rotate-180' : 'rotate-0'" viewBox="0 0 12 12">
                                        <path d="M5.9 11.4L.5 6l1.4-1.4 4 4 4-4L11.3 6z" />
                                    </svg>
                                </div>
                                <!-- <div class="flex ub nq ttw tnn 2xl:opacity--100 wr">
                        ...
                        <div class="tex ttj 2xl:block">
                            <ul class="me re @if(!in_array(Request::segment(2), ['faqs', 'slides', 'reports', 'subscribers'])){{ 'hidden' }}@else{ !block }@endif" :class="open ? '!block' : 'hidden'">
                                <li class="rt ww">
