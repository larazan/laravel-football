        ...
        <div class="ff">
            ...
            <div>
                ...
                <ul class="nk">
                    ...
                    <!-- Trainings -->
                    <li class="vn vr rounded-sm n_ ww @if(in_array(Request::segment(2), ['trainings', 'training-type', 'training-session'])){{ 'bg-slate-900' }}@else{{ '' }}@endif" x-data="{ open: false }">
                        <a class="block gj xc ld wt wi" href="#0" @click.prevent="sidebarExpanded ? open = !open : sidebarExpanded = true">
                            <div class="flex items-center fe">
                                <div class="flex items-center">
                                    <svg class="ub so oi" viewBox="0 0 24 24">
                                        <path class="du @if(in_array(Request::segment(2), ['trainings', 'training-type', 'training-session'])){{ 'text-indigo-500' }}@else{{ 'g_' }}@endif" d="M19 5h1v14h-2V7.414L5.707 19.707 5 19H4V5h2v11.586L18.293 4.293 19 5Z"></path>
                                        <path class="du @if(in_array(Request::segment(2), ['trainings', 'training-type', 'training-session'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" d="M5 9a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm14 0a4 4 0 1 1 0-8 4 4 0 0 1 0 8ZM5 23a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm14 0a4 4 0 1 1 0-8 4 4 0 0 1 0 8Z"></path>
                                    </svg>
                                    <!-- <svg class="ub so oi" viewBox="0 0 24 24">
                                        <path class="du @if(in_array(Request::segment(2), ['womens', 'unders', 'squads'])){{ 'text-indigo-500' }}@else{{ 'g_' }}@endif" d="M18.974 8H22a2 2 0 012 2v6h-2v5a1 1 0 01-1 1h-2a1 1 0 01-1-1v-5h-2v-6a2 2 0 012-2h.974zM20 7a2 2 0 11-.001-3.999A2 2 0 0120 7zM2.974 8H6a2 2 0 012 2v6H6v5a1 1 0 01-1 1H3a1 1 0 01-1-1v-5H0v-6a2 2 0 012-2h.974zM4 7a2 2 0 11-.001-3.999A2 2 0 014 7z"></path>
                                        <path class="du @if(in_array(Request::segment(2), ['womens', 'unders', 'squads'])){{ 'text-indigo-300' }}@else{{ 'gq' }}@endif" d="M12 6a3 3 0 110-6 3 3 0 010 6zm2 18h-4a1 1 0 01-1-1v-6H6v-6a3 3 0 013-3h6a3 3 0 013 3v6h-3v6a1 1 0 01-1 1z"></path>
                                    </svg> -->
                                    <span class="text-sm gp ml-3 ttw tnn 2xl:opacity--100 wr">Trainings</span>
                                </div>
                                <!-- Icon -->
                                <div class="flex shrink-0 ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">
                                    <svg class="w-3 h-3 shrink-0 ml-1 fill-current text-slate-400 @if(in_array(Request::segment(2), ['trainings', 'training-type', 'training-session'])){{ 'rotate-180' }}@endif" :class="open ? 'rotate-180' : 'rotate-0'" viewBox="0 0 12 12">
                                        <path d="M5.9 11.4L.5 6l1.4-1.4 4 4 4-4L11.3 6z" />
                                    </svg>
                                </div>
                            </div>
                        ...
                        <div class="tex ttj 2xl:block">
                            <ul class="me re @if(!in_array(Request::segment(2), ['trainings', 'training-type', 'training-session'])){{ 'hidden' }}@else{ !block }@endif" :class="open ? '!block' : 'hidden'">
                                <li class="rt ww">
                                    <a class="block @if(in_array(Request::segment(2), ['trainings'])){{ 'text-indigo-500' }}@else{{ 'gq hover--text-slate-200' }}@endif wt wi ld" href="{{ url('admin/trainings') }}">
                                        <span class="text-sm gp ttw tnn 2xl:opacity--100 wr">Trainings</span>
                                    </a>
                                </li>
                                <li class="rt ww">
                                    <a class="block @if(in_array(Request::segment(2), ['training-type'])){{ 'text-indigo-500' }}@else{{ 'gq hover--text-slate-200' }}@endif wt wi ld" href="{{ url('admin/training-type') }}">
                                        <span class="text-sm gp ttw tnn 2xl:opacity--100 wr">Training Types</span>
                                    </a>
                                </li>
                                <li class="rt ww">
                                    <a class="block @if(in_array(Request::segment(2), ['training-session'])){{ 'text-indigo-500' }}@else{{ 'gq hover--text-slate-200' }}@endif wt wi ld" href="{{ url('admin/training-session') }}">
                                        <span class="text-sm gp ttw tnn 2xl:opacity--100 wr">Training Sessions</span>
                                    </a>
                                </li>
